require(['common', 'echarts3','echartsConfig', 'jquery', 'commonEditor', 'httpRequest', 'ace/ace', 'ace/ext/language_tools'], function (common, echarts, echartsConfig, $, commonEditor, http, ace) {
  console.log(ace)
  ...
  function formatEchartData () {
    ...
    var xAxisData = [];
    var series = [];
    data.forEach(function(item, typeIndex) {
      ...
      if (xAxisData.length < 1) {
        item.categories.forEach(function(cateItem) {
          ...
          console.log(value)
          seriesItem.xAxisIndex = seriesItem.yAxisIndex = typeIndex == 0 ? 0 : 2;
          console.log(seriesItem)
          seriesItem.itemStyle.normal.label.position = typeIndex == 0 ? 'left' : 'right';
        });
      }else {
        item.categories.forEach(function(cateItem) {
          var category = cateItem.category;
  ...
  function updateEchart() {
    ...
    option.yAxis[1].data = formatData.yAxisData.map(function(value) {
      return {
        value: value,
        textStyle: {
          align: 'center',
    ...
    option.yAxis[2].data = formatData.yAxisData;
